  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('https://b99b-39-36-65-61.ngrok-free.app/meta/fetch_account_details', {
          method: 'POST',
          body: formData
        })
      ...
      <AnimatePresence>
        ...
        >
          <OverviewCard
            ...
            change={formatPercentage(accountDetails?.equity && accountDetails?.balance 
              ? ((accountDetails.equity - accountDetails.balance) / accountDetails.balance * 100)
              : 0
            )}
        ...
        >
          <Card className="p-6 bg-gradient-to-br from-[#0d2339]/90 to-[#132f4c]/90 border-gray-800/50 backdrop-blur-sm">
            <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="relative w-32 h-32">
                      <svg className="w-full h-full" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeDashoffset={251.2 * (1 - drawdownData.recoveryProgress / 100)}
                          strokeLinecap="round"
                          stroke="currentColor"
                          fill="transparent"
                          r="40"
                      ...
                      <div className="absolute inset-0 flex items-center justify-center flex-col">
                        <span className="text-2xl font-bold text-white">
                          {drawdownData.currentDrawdown}%
                        </span>
                        <span className="text-sm text-gray-400">Current DD</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 grid grid-cols-2 gap-4">
                ...
                <DrawdownStat
                  ...
                  value={`${drawdownData.recoveryProgress}%`}
                  trend="up"
                  color="text-green-400"
                />
                <DrawdownStat
                  label="DD Start Date"
                  value={new Date(drawdownData.startDate).toLocaleDateString()}
                  color="text-blue-400"
                />
                <DrawdownStat
                  label="Lowest Point"
                  value={new Date(drawdownData.lowestPoint).toLocaleDateString()}
  ...
  return (
    <div className="bg-white/5 rounded-lg p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        {trend && (
          <div className={`${color} ${trend === 'up' ? 'rotate-0' : 'rotate-180'}`}>
            <svg
              className="w-4 h-4"
              fill="none"
              stroke="currentColor"
